...
Reusable **GitHub Actions** for keyless Terraform CI/CD. A consumer repo calls one reusable
workflow (or composes the building-block actions) and gets a standardized **detect → plan →
apply** flow with per-stack OIDC federation and environment-based approval gating.

> Generic and vendor-neutral by design — no organization, product, or account values are baked
> into this catalog. Identity lives per-stack in your repo's `tf-ci.env` files.
...
| [`detect-changes`](actions/detect-changes) | Map changed files to Terraform workspaces → job matrix |
