...
for c in tf-env tf-run aws-oidc detect-changes secret-scan tf-lint resolve-env tf-docs cfn-env cfn-run cfn-lint checkout-artifact registry-login docker-image; do
  TAGS="${TAGS} ${c}/v${MAJOR}"
done
